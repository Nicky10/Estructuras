...
public class SearchAlgoritms {
	...
	{
		if (upperBound < lowerBound && upperBound != value) 
                    return -1;

                int index = lowerBound + (upperBound - 1) * (value - arrayData[lowerBound]) / (arrayData[upperBound] - arrayData[lowerBound]);

                if (value < arrayData[index]) 
                    return interpolationSearch_recursive(arrayData, value, lowerBound, index - 1);
                else 
                    if (arrayData[index] < value)
                        return interpolationSearch_recursive(arrayData, value, index + 1, upperBound);
                    else return index;
