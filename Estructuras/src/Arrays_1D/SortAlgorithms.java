...
public class SortAlgorithms {
	...
	public int[] quickSort(int[] unsorted, int left, int right) 
        {
            ...
            {            
                while(unsorted[izq_der] <= pivote && izq_der < der_izq) 
                    izq_der++; 
                while(unsorted[der_izq]>pivote) 
                    der_izq--;         
                if (izq_der<der_izq) 
                {                                            
                    aux= unsorted[izq_der];                  
                    unsorted[izq_der]=unsorted[der_izq];
                    unsorted[der_izq]=aux;
                }
            }
            
            unsorted[left]=unsorted[der_izq]; 
            unsorted[der_izq]=pivote; 
	 ...
	 * @return
	 */
	public int[] randomQsuickSort(int[] unsorted, int left, int right)
	{
            Random rd = new Random();
            int pivote=(int) Math.floor(Math.random()*(unsorted[right]-unsorted[left]+1)+unsorted[left]);
            int izq_der=left; 
            int der_izq=right; 
            int aux;
	...
	public static void main(String[] args)
	{
		...
		a = sorts.quickSort(a,0,a.length - 1);
		bo.printArray(a);
	}
}